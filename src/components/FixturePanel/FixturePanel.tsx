...
import {
  getEffectivePredictions,
  getLiveScoreMatchIds,
...
export const FixturePanel = memo(
  ({
    ...
    const dateGroups = useGroupedMatches(
      matches,
      predictions,
      teamsById,
      groupOptions,
    );
    const teamGroups = useTeamGroupedMatches(
      matches,
      predictions,
      teamsById,
      groupOptions,
    ...
    const sortedTeamGroups = useMemo(
      () =>
        [...teamGroups].sort((a, b) => {
          const posA = a.team
            ? (standingPositionsByTeamId.get(a.team.id) ?? Infinity)
            : Infinity;
          const posB = b.team
            ? (standingPositionsByTeamId.get(b.team.id) ?? Infinity)
            : Infinity;
          return posA - posB;
        }),
