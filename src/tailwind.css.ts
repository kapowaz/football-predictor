...
export const createGlobalVarColorSteps = (
  color: ColorHue,
): Record<ColorStep, ReturnType<typeof createGlobalVar>> => {
  return COLOR_STEPS.reduce(
    (acc, step) => {
      acc[step] = createGlobalVar(`color-${color}-${step}`);
      return acc;
    },
    {} as Record<number, ReturnType<typeof createGlobalVar>>,
  );
};

const colors = COLOR_HUES.reduce(
  (acc, hue) => {
    acc[hue] = createGlobalVarColorSteps(hue);
    return acc;
  },
  {} as Record<ColorHue, ReturnType<typeof createGlobalVarColorSteps>>,
);
