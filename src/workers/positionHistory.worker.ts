...
self.addEventListener('message', (event: MessageEvent<PositionHistoryRequest>) => {
  const { requestId, teams, matches, predictions, deductions, variantRules } = event.data;
  const map = calculatePositionHistory(teams, matches, predictions, deductions, variantRules);
