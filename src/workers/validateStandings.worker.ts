...
self.addEventListener('message', (event: MessageEvent<ValidateStandingsRequest>) => {
  const { requestId, teams, matches, defaultDeductions, apiStandings } = event.data;

  // Capture console output
  const logs: ValidateStandingsResponse['logs'] = [];
  const origLog = console.log;
  const origWarn = console.warn;
  console.log = (...args: unknown[]) => logs.push({ level: 'log', args: args.map(String) });
  console.warn = (...args: unknown[]) => logs.push({ level: 'warn', args: args.map(String) });

  try {
    const emptyPredictions = { predictions: {}, lastModified: '' };
    const calculated = calculateStandings(teams, matches, emptyPredictions, defaultDeductions);
