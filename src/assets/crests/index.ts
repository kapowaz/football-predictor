import accringtonStanley from './accrington-stanley.svg';
import afcBournemouth from './afc-bournemouth.svg';
import afcWimbledon from './afc-wimbledon.svg';
import arsenal from './arsenal.svg';
import astonVilla from './aston-villa.svg';
import barnet from './barnet.svg';
import barnsley from './barnsley.svg';
import barrow from './barrow.svg';
import birminghamCity from './birmingham-city.svg';
import blackburnRovers from './blackburn-rovers.svg';
import blackpool from './blackpool.svg';
import boltonWanderers from './bolton-wanderers.svg';
import bradfordCity from './bradford-city.svg';
import brentford from './brentford.svg';
import brightonAndHoveAlbion from './brighton.svg';
import bristolCity from './bristol-city.svg';
import bristolRovers from './bristol-rovers.svg';
import bromley from './bromley.svg';
import burnley from './burnley.svg';
import burtonAlbion from './burton-albion.svg';
import cambridgeUnited from './cambridge-united.svg';
import cardiffCity from './cardiff-city.svg';
import charltonAthletic from './charlton-athletic.svg';
import cheltenhamTown from './cheltenham-town.svg';
import chelsea from './chelsea.svg';
import chesterfield from './chesterfield.svg';
import colchesterUnited from './colchester-united.svg';
import coventryCity from './coventry-city.svg';
import crawleyTown from './crawley-town.svg';
import creweAlexandra from './crewe-alexandra.svg';
import crystalPalace from './crystal-palace.svg';
import derbyCounty from './derby-county.svg';
import doncasterRovers from './doncaster-rovers.svg';
import everton from './everton.svg';
import exeterCity from './exeter-city.svg';
import fleetwoodTown from './fleetwood-town.svg';
import fulham from './fulham.svg';
import gillingham from './gillingham.svg';
import grimsbyTown from './grimsby-town.svg';
import harrogate from './harrogate-town.svg';
import huddersfieldTown from './huddersfield-town.svg';
import hullCity from './hull-city.svg';
import ipswichTown from './ipswich-town.svg';
import leedsUnited from './leeds-united.svg';
import leicesterCity from './leicester-city.svg';
import leytonOrient from './leyton-orient.svg';
import lincolnCity from './lincoln-city.svg';
import liverpool from './liverpool.svg';
import lutonTown from './luton-town.svg';
import manchesterCity from './manchester-city.svg';
import manchesterUnited from './manchester-united.svg';
import mansfieldTown from './mansfield-town.svg';
import middlesbrough from './middlesbrough.svg';
import millwall from './millwall.svg';
import miltonKeynesDons from './milton-keynes-dons.svg';
import newcastleUnited from './newcastle-united.svg';
import newportCounty from './newport-county.svg';
import northamptonTown from './northampton-town.svg';
import norwichCity from './norwich-city.svg';
import nottinghamForest from './nottingham-forest.svg';
import nottsCounty from './notts-county.svg';
import oldhamAthletic from './oldham-athletic.svg';
import oxfordUnited from './oxford-united.svg';
import peterboroughUnited from './peterborough-united.svg';
import plymouthArgyle from './plymouth-argyle.svg';
import portVale from './port-vale.svg';
import portsmouth from './portsmouth.svg';
import prestonNorthEnd from './preston-north-end.svg';
import queensParkRangers from './queens-park-rangers.svg';
import reading from './reading.svg';
import rotherhamUnited from './rotherham-united.svg';
import salfordCity from './salford-city.svg';
import sheffieldUnited from './sheffield-united.svg';
import sheffieldWednesday from './sheffield-wednesday.svg';
import shrewsburyTown from './shrewsbury-town.svg';
import southampton from './southampton.svg';
import stevenage from './stevenage.svg';
import stockportCounty from './stockport-county.svg';
import stokeCity from './stoke-city.svg';
import sunderland from './sunderland.svg';
import swanseaCity from './swansea-city.svg';
import swindonTown from './swindon-town.svg';
import tottenhamHotspur from './tottenham-hotspur.svg';
import tranmereRovers from './tranmere-rovers.svg';
import walsall from './walsall.svg';
import watford from './watford.svg';
import westBromwichAlbion from './west-bromwich-albion.svg';
import westHamUnited from './west-ham-united.svg';
import wiganAthletic from './wigan-athletic.svg';
import wolverhamptonWanderers from './wolverhampton-wanderers.svg';
import wrexham from './wrexham.svg';
import wycombeWanderers from './wycombe-wanderers.svg';

const crests: Record<string, string> = {
  'accrington-stanley': accringtonStanley,
  'afc-bournemouth': afcBournemouth,
  'afc-wimbledon': afcWimbledon,
  arsenal: arsenal,
  'aston-villa': astonVilla,
  barnet: barnet,
  barnsley: barnsley,
  barrow: barrow,
  'birmingham-city': birminghamCity,
  'blackburn-rovers': blackburnRovers,
  blackpool: blackpool,
  'bolton-wanderers': boltonWanderers,
  'bradford-city': bradfordCity,
  brentford: brentford,
  'brighton-and-hove-albion': brightonAndHoveAlbion,
  'bristol-city': bristolCity,
  'bristol-rovers': bristolRovers,
  bromley: bromley,
  burnley: burnley,
  'burton-albion': burtonAlbion,
  'cambridge-united': cambridgeUnited,
  'cardiff-city': cardiffCity,
  'charlton-athletic': charltonAthletic,
  'cheltenham-town': cheltenhamTown,
  chelsea: chelsea,
  chesterfield: chesterfield,
  'colchester-united': colchesterUnited,
  'coventry-city': coventryCity,
  'crawley-town': crawleyTown,
  'crewe-alexandra': creweAlexandra,
  'crystal-palace': crystalPalace,
  'derby-county': derbyCounty,
  'doncaster-rovers': doncasterRovers,
  everton: everton,
  'exeter-city': exeterCity,
  'fleetwood-town': fleetwoodTown,
  fulham: fulham,
  gillingham: gillingham,
  'grimsby-town': grimsbyTown,
  'harrogate-town': harrogate,
  'huddersfield-town': huddersfieldTown,
  'hull-city': hullCity,
  'ipswich-town': ipswichTown,
  'leeds-united': leedsUnited,
  'leicester-city': leicesterCity,
  'leyton-orient': leytonOrient,
  'lincoln-city': lincolnCity,
  liverpool: liverpool,
  'luton-town': lutonTown,
  'manchester-city': manchesterCity,
  'manchester-united': manchesterUnited,
  'mansfield-town': mansfieldTown,
  middlesbrough: middlesbrough,
  millwall: millwall,
  'milton-keynes-dons': miltonKeynesDons,
  'newcastle-united': newcastleUnited,
  'newport-county': newportCounty,
  'northampton-town': northamptonTown,
  'norwich-city': norwichCity,
  'nottingham-forest': nottinghamForest,
  'notts-county': nottsCounty,
  'oldham-athletic': oldhamAthletic,
  'oxford-united': oxfordUnited,
  'peterborough-united': peterboroughUnited,
  'plymouth-argyle': plymouthArgyle,
  'port-vale': portVale,
  portsmouth: portsmouth,
  'preston-north-end': prestonNorthEnd,
  'queens-park-rangers': queensParkRangers,
  reading: reading,
  'rotherham-united': rotherhamUnited,
  'salford-city': salfordCity,
  'sheffield-united': sheffieldUnited,
  'sheffield-wednesday': sheffieldWednesday,
  'shrewsbury-town': shrewsburyTown,
  southampton: southampton,
  stevenage: stevenage,
  'stockport-county': stockportCounty,
  'stoke-city': stokeCity,
  sunderland: sunderland,
  'swansea-city': swanseaCity,
  'swindon-town': swindonTown,
  'tottenham-hotspur': tottenhamHotspur,
  'tranmere-rovers': tranmereRovers,
  walsall: walsall,
  watford: watford,
  'west-bromwich-albion': westBromwichAlbion,
  'west-ham-united': westHamUnited,
  'wigan-athletic': wiganAthletic,
  'wolverhampton-wanderers': wolverhamptonWanderers,
  wrexham: wrexham,
  'wycombe-wanderers': wycombeWanderers,
};

export const getCrest = (crestKey: string): string => {
  return crests[crestKey] ?? '';
};

export default crests;
